...
import pjt.one.com.dao.BoardDao00;
import pjt.one.com.vo.BoardListVo;


@WebServlet("/board00")
public class BoardController00 extends HttpServlet {
	...
	private void requestFunc(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(cmd.equals("FIRSTLIST")) { 
			BoardDao00 dao= new BoardDao00();
			List<BoardListVo> boardList = dao.getBoardList();
			request.setAttribute("boardList", boardList);
			
			String link = "/view00/list00.jsp";
		...
		if(cmd.equals("BOARDWRITEFORM")) { 
			String link = "/view00/write00.jsp"; 
			request.getRequestDispatcher(link).forward(request, response);
		}		
		if(cmd.equals("BOARDWRITE")) {	
		 ...
		 String link = "/board00?cmd=FIRSTLIST"; 
		 request.getRequestDispatcher(link).forward(request, response);
		...
		if(cmd.equals("BOARDUPDATE")){
			...
			out.println("<script>alert('UPDATE OK'); location.href='/board00?cmd=FIRSTLIST';</script>");
			out.flush();
		}
